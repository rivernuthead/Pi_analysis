...
for set_name in set_names:
    ...
    output_folder = os.path.join(home_dir,'output','report_'+set_name, 'envelopes')
    ...
    for d in range(0,stack.shape[3]):
        ...
        np.savetxt(os.path.join(output_folder, set_name + '_timespan'+ str(d) + '_envMAW_report.txt'), envMAA_report, delimiter=',', header=header, fmt=fmt_string)