...
# FOLDER SETUP ----------------------------------------------------------------
home_dir = 'e:\\repos\\pi_analysis\\morphological_analysis' # Home directory
...
for i, set_name in enumerate(set_names):
    ...
    envBAW_mean_std = np.loadtxt(os.path.join('e:\\repos\\pi_analysis\\', 'bedload_analysis','output_data', 'output_report', BAW_set_name + '_envBAW_merged_single_runs.txt'), delimiter=',', skiprows=0)
    ...
    # Plotting the data
    # plt.errorbar(envBAW_mean_std[0, :], envBAW_mean_std[1, :], yerr=envBAW_mean_std[2, :], fmt='s', linestyle='--', 
    #              ecolor='black', color='red', capsize=2, label='envBAW Mean ± Std', linewidth=1.5, markersize=4)
    plt.errorbar(MAW_x_values, envMAW[:, 1], yerr=envMAW[:, 2], label='envMAW', color='blue', capsize=2, marker='o', 
                 linewidth=1.5, markersize=4)
    plt.errorbar(envBAW_mean_std[0, :], envBAW_mean_std[1, :], yerr=envBAW_mean_std[2, :], fmt='s', linestyle='--', 
                 ecolor='black', color='red', capsize=2, label='envBAW', linewidth=1.5, markersize=4)
    plt.errorbar(MAW_x_values, MAW_mean_std[0, :], yerr=MAW_mean_std[1, :], fmt='s', linestyle='--', 
                 ecolor='black', color='gray', capsize=2, label='MAW', linewidth=1.5, markersize=4)
    plt.errorbar(comp_thrs_x_values, compensation_mean_array, yerr=compensation_std_array, fmt='o', linestyle='-', 
                 ecolor='black', color='green', capsize=2, label='Compensation', linewidth=1.5, markersize=4)
    plt.errorbar(comp_thrs_x_values, threshold_pure_mean_array, yerr=threshold_pure_std_array, fmt='o', linestyle='-', 
                 ecolor='black', color='purple', capsize=2, label='Thrs and filt', linewidth=1.5, markersize=4)
    
    # Labels, title, legend, and customization
    plt.xlabel('t / Exner time [-]', fontsize=10)
    plt.ylabel('Normalized active width [-]', fontsize=10)
    plt.title(f'{set_name}', fontsize=12)
    ...
    plt.savefig(os.path.join(home_dir, 'output_data', f'comp_thrs_filt_plot_{set_name}.pdf'), 
                dpi=800, bbox_inches='tight')
    plt.show()


    
    # CREATE AND SAVE CHART REPORT
    report_lines = []
...
for i, set_name in enumerate(set_names):
    ...
    # Plotting the data
    # plt.errorbar(envBAW_mean_std[0, :], envBAW_mean_std[1, :], yerr=envBAW_mean_std[2, :], fmt='s', linestyle='--', ecolor='black', capsize=2, label='envBAW Mean ± Std', linewidth=1.5, markersize=4)
    # plt.errorbar(MAW_x_values, envMAW[:, 1], yerr=envMAW[:, 2], label=disch, color='blue', capsize=2, marker='o', linewidth=1.5, markersize=4)
    # plt.errorbar(envBAW_mean_std[0, :], envBAW_mean_std[1, :], yerr=envBAW_mean_std[2, :], fmt='s', linestyle='--', ecolor='black', capsize=2, label=disch, linewidth=1.5, markersize=4)
    plt.errorbar(MAW_x_values, MAW_mean_std[0, :], yerr=MAW_mean_std[1, :], fmt='s', linestyle='--', ecolor='black', capsize=2, label=disch, linewidth=1.5, markersize=4)
    # plt.errorbar(comp_thrs_x_values, compensation_mean_array, yerr=compensation_std_array, fmt='o', linestyle='-', ecolor='black',  capsize=2, label=disch, linewidth=1.5, markersize=4)
    # plt.errorbar(comp_thrs_x_values, threshold_pure_mean_array, yerr=threshold_pure_std_array, fmt='o', linestyle='-', ecolor='black', capsize=2, label=disch, linewidth=1.5, markersize=4)
    
# Labels, title, legend, and customization
plt.xlabel('t / Exner time [-]', fontsize=10)
plt.ylabel('Normalized active width [-]', fontsize=10)
plt.title('Morphological active width', fontsize=12)
plt.legend(bbox_to_anchor=(0, 1), loc='upper left', fontsize=8)
plt.ylim(0.0, 1.0)
plt.grid(True, linestyle='--', alpha=0.7)
...
# Saving and showing the plot
plt.savefig(os.path.join(home_dir, 'output_data', 'Overall_MAW_over_time.pdf'), 
            dpi=800, bbox_inches='tight')
plt.show()
