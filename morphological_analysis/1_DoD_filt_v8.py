...
In this v8 version, 6th step (RE-PERFORM ISOLATED PIXEL REMOVAL) has been modified. Instead of performing the remove_small_object function considering both erosion and deposition areas, it is applied on erosion and depostion areas separately. Then, the 2 arrays are merged together to continue the processing as past versions.
"""
import os
import shutil
import time
...
thrs_nature = 3 # [-] nature_checker function threshold
thrs_fill = 7 # [-] isolated_filler function threshold
thrs_1 = 1.3  # [mm] # Lower threshold
thrs_2 = 15.0  # [mm] # Upper threshold
thrs_isol2 = 30 # minimum object size to keep during the second remove_small_object operation
...
#%%
    ...
    for h in range (0, len(files)-1):
        for k in range (0, len(files)-1-h):
            ...
            # 6 - PERFORM ISOLATED PIXEL REMOVAL on both erosion and deposition areas separately.
            # 6a- PERFORM ISOLATED PIXEL REMOVAL on erosion areas:
            #-----------------------------------------------------
            # After trimming al np.nan values, counter represent the number of
            # pixel not equal to zero o fthe DoD_filt_fill matrix
            DoD_filt_nature_erosion = np.where(DoD_filt_nature>0, 0, DoD_filt_nature)
            counter0 = np.count_nonzero(DoD_filt_nature_erosion[np.logical_not(np.isnan(DoD_filt_nature_erosion))])
            
            # Perform the very first isolated_killer procedure
            DoD_filt_isol2_erosion = remove_small_objects(DoD_filt_nature_erosion, thrs_isol2, 2)
            
            # After trimming al np.nan values, counter represent the number of
            # pixel not equal to zero of the DoD_filt_isol2 matrix
            counter1 = np.count_nonzero(DoD_filt_isol2_erosion[np.logical_not(np.isnan(DoD_filt_isol2_erosion))])
            # Perform the isolated_killer procedure until the number of active
            # pixel will not change anymore

            while counter0-counter1!=0:
                # Filtering...
                DoD_filt_isol2 = remove_small_objects(DoD_filt_isol2_erosion, thrs_isol2, 2)
                # Update counters:
                counter0 = counter1
                counter1 = np.count_nonzero(DoD_filt_isol2_erosion[np.logical_not(np.isnan(DoD_filt_isol2_erosion))])

            DoD_filt_isol2_erosion_gis = np.where(np.isnan(DoD_filt_isol2_erosion), NaN, DoD_filt_isol2_erosion)
            
            DoD_filt_isol2_erosion_plot = rescaling_plot(DoD_filt_isol2_erosion)

            # 6b- PERFORM ISOLATED PIXEL REMOVAL on deposition areas:
            #-----------------------------------------------------
            # After trimming al np.nan values, counter represent the number of
            # pixel not equal to zero o fthe DoD_filt_fill matrix
            DoD_filt_nature_deposition = np.where(DoD_filt_nature<0, 0, DoD_filt_nature)
            counter0 = np.count_nonzero(DoD_filt_nature_deposition[np.logical_not(np.isnan(DoD_filt_nature_deposition))])
            
            # Perform the very first isolated_killer procedure
            DoD_filt_isol2_deposition = remove_small_objects(DoD_filt_nature_deposition, thrs_isol2, 2)
            
            # After trimming al np.nan values, counter represent the number of
            # pixel not equal to zero of the DoD_filt_isol2 matrix
            counter1 = np.count_nonzero(DoD_filt_isol2_deposition[np.logical_not(np.isnan(DoD_filt_isol2_deposition))])
            # Perform the isolated_killer procedure until the number of active
            # pixel will not change anymore

            while counter0-counter1!=0:
                # Filtering...
                DoD_filt_isol2 = remove_small_objects(DoD_filt_isol2, thrs_isol2, 2)
                # Update counters:
                counter0 = counter1
                counter1 = np.count_nonzero(DoD_filt_isol2_deposition[np.logical_not(np.isnan(DoD_filt_isol2_deposition))])

            DoD_filt_isol2_deposition_gis = np.where(np.isnan(DoD_filt_isol2_deposition), NaN, DoD_filt_isol2_deposition)
            
            DoD_filt_isol2_deposition_plot = rescaling_plot(DoD_filt_isol2_deposition)
            
            #Merge both erosion and deposition
            DoD_filt_isol2 = np.copy(DoD_filt_nature)
            DoD_filt_isol2 = np.where(DoD_filt_isol2_deposition>0, DoD_filt_isol2_deposition,0)
            DoD_filt_isol2 = np.where(DoD_filt_isol2_erosion<0, DoD_filt_isol2_erosion,DoD_filt_isol2)
            DoD_filt_isol2 = np.where(np.isnan(DoD_filt_nature)==True, np.nan, DoD_filt_isol2)
            ...
            # 7- PERFORM ISOLATED PIXEL REMOVAL:
            #-----------------------------------------------------
            ...
            # 8- RE-PERFORM ISOLATED ANISOTROPIC PIXEL REMOVAL:
            #-----------------------------------------------------
            # After trimming al np.nan values, counter represent the number of
            # pixel not equal to zero o fthe DoD_filt_fill matrix
...
#%%         ###################################################################
            ...
            if filt_analysis == 1:
                ...
                if delta == 1:
                    # DoD_raw
                    tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep, morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri = morph_quantities(
                        DoD_raw)
                    if len(DoD_raw_morph_quant) == 0:
                        DoD_raw_morph_quant = np.append(DoD_raw_morph_quant, (tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                        morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))
                    else:
                        DoD_raw_morph_quant = np.vstack((DoD_raw_morph_quant, np.hstack((tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                        morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))))
                    ...
                    if len(DoD_filt_mean_morph_quant) == 0:
                        DoD_filt_mean_morph_quant = np.append(DoD_filt_mean_morph_quant, (tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                              morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))
                    else:
                        DoD_filt_mean_morph_quant = np.vstack((DoD_filt_mean_morph_quant, np.hstack((tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                              morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))))
                    ...
                    if len(DoD_filt_isol_morph_quant) == 0:
                        DoD_filt_isol_morph_quant = np.append(DoD_filt_isol_morph_quant, (tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                              morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))
                    else:
                        DoD_filt_isol_morph_quant = np.vstack((DoD_filt_isol_morph_quant, np.hstack((tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                              morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))))
                    ...
                    if len(DoD_filt_fill_morph_quant) == 0:
                        DoD_filt_fill_morph_quant = np.append(DoD_filt_fill_morph_quant, (tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                              morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))
                    else:
                        DoD_filt_fill_morph_quant = np.vstack((DoD_filt_fill_morph_quant, np.hstack((tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                              morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))))
                    ...
                    if len(DoD_filt_nature_morph_quant) == 0:
                        DoD_filt_nature_morph_quant = np.append(DoD_filt_nature_morph_quant, (tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                                morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))
                    else:
                        DoD_filt_nature_morph_quant = np.vstack((DoD_filt_nature_morph_quant, np.hstack((tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                                morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))))
                    ...
                    if len(DoD_filt_isol2_morph_quant) == 0:
                        DoD_filt_isol2_morph_quant = np.append(DoD_filt_isol2_morph_quant, (tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                               morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))
                    else:
                        DoD_filt_isol2_morph_quant = np.vstack((DoD_filt_isol2_morph_quant, np.hstack((tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                               morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))))
                    ...
                    if len(DoD_filt_ult_morph_quant) == 0:
                        DoD_filt_ult_morph_quant = np.append(DoD_filt_ult_morph_quant, (tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                             morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))
                    else:
                        DoD_filt_ult_morph_quant = np.vstack((DoD_filt_ult_morph_quant, np.hstack((tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep,
                                                             morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri))))
            ...
            # Print filtered DoD (with np.nan)...
            np.savetxt(os.path.join(path_out, DoD_name + 'filt_isol2_erosion.txt'), DoD_filt_isol2_erosion, fmt='%0.1f', delimiter='\t')
            # Print filtered DoD (with NaN as -999)
            np.savetxt(os.path.join(path_out, DoD_name + 'filt_isol2_erosion_gis.txt'), DoD_filt_isol2_erosion_gis, fmt='%0.1f', delimiter='\t')
            
            # SECOND ROUND OF ISOLATE KILLER FUNCTION APPLIED DoD (This is the ultimate DoD)
            # Print filtered DoD (with np.nan)...
            np.savetxt(os.path.join(path_out, DoD_name + 'filt_isol2_deposition.txt'), DoD_filt_isol2_deposition, fmt='%0.1f', delimiter='\t')
            # Print filtered DoD (with NaN as -999)
            np.savetxt(os.path.join(path_out, DoD_name + 'filt_isol2_deposition_gis.txt'), DoD_filt_isol2_deposition_gis, fmt='%0.1f', delimiter='\t')
            ...
            with open(os.path.join(path_out, DoD_name + 'filt_isol2_erosion_gis.txt')) as f_DoD_isol2_erosion:
                w_DoD_isol2_erosion = f_DoD_isol2_erosion.read()
            with open(os.path.join(path_out, DoD_name + 'filt_isol2_deposition_gis.txt')) as f_DoD_isol2_deposition:
                w_DoD_isol2_deposition = f_DoD_isol2_deposition.read()
            ...
            with open(os.path.join(path_out, DoD_name + 'activity_map_sco_gis.txt')) as f_DoD_act_map_sco:
                ...
                DoD_isol2_erosion_gis = w_header + w_DoD_isol2_erosion
                DoD_isol2_deposition_gis = w_header + w_DoD_isol2_deposition
            ...
            with open(os.path.join(path_out, DoD_name + 'filt_isol2_erosion_gis.txt'), 'w') as fp:
                fp.write(DoD_isol2_erosion_gis)
            with open(os.path.join(path_out, DoD_name + 'filt_isol2_deposition_gis.txt'), 'w') as fp:
                fp.write(DoD_isol2_deposition_gis)    
    ...
    if filt_analysis==1:
        ...
        for matrix in (DoD_raw_morph_quant, DoD_filt_mean_morph_quant, DoD_filt_isol_morph_quant,DoD_filt_fill_morph_quant,DoD_filt_nature_morph_quant,DoD_filt_isol2_morph_quant,DoD_filt_ult_morph_quant):
            ...
            np.savetxt(os.path.join(report_dir, 'morph_quant_report.txt'), matrix_stack,
                       header='tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep, morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri')

        # Save report for the analysis of the effects of spatial filter application on DoD, at different stages.
        np.savetxt(os.path.join(report_dir, 'DoD_raw_morph_quant.txt'), DoD_raw_morph_quant,
                   header='tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep, morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri')
        np.savetxt(os.path.join(report_dir, 'DoD_filt_mean_morph_quant.txt'), DoD_filt_mean_morph_quant,
                   header='tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep, morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri')
        np.savetxt(os.path.join(report_dir, 'DoD_filt_isol_morph_quant.txt'), DoD_filt_isol_morph_quant,
                   header='tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep, morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri')
        np.savetxt(os.path.join(report_dir, 'DoD_filt_fill_morph_quant.txt'), DoD_filt_fill_morph_quant,
                   header='tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep, morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri')
        np.savetxt(os.path.join(report_dir, 'DoD_filt_nature_morph_quant.txt'), DoD_filt_nature_morph_quant,
                   header='tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep, morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri')
        np.savetxt(os.path.join(report_dir, 'DoD_filt_isol2_morph_quant.txt'), DoD_filt_isol2_morph_quant,
                   header='tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep, morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri')
        np.savetxt(os.path.join(report_dir, 'DoD_filt_ult_morph_quant.txt'), DoD_filt_ult_morph_quant,
                   header='tot_vol, sum_vol, dep_vol, sco_vol, morph_act_area, morph_act_area_dep, morph_act_area_sco, act_width_mean, act_width_mean_dep, act_width_mean_sco, act_thickness, act_thickness_dep, act_thickness_sco, bri')
