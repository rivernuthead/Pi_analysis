...
stack_dir = os.path.join(home_dir, 'output', 'DoDs', 'DoDs_stack')
...
mask = tiff.imread('mask_DEM_DoD_envBAA.tiff')
...
envBAA_act_period_list = [] # Collect all the activity period as fraction of the run duration
...
# =============================================================================
    ...
    if 'full_BAA_stack' in run_mode:
        envBAA_act_period = np.load(os.path.join('/home/erri/Documents/PhD/Research/5_research_repos/PiQs_analysis/activity_stack/', run_name + '_envBAA_act_period_LR5.npy'))
    ...
    DEM_stack = np.load(os.path.join(home_dir, 'surveys',set_name, set_name + '_DEM_stack.npy'))
    ...
    DoD_stack = np.load(os.path.join(home_dir, 'output','DoDs', 'DoDs_stack','DoD_stack_' + set_name + '.npy'))
    DoD_stack = DoD_stack[:,:,:,0] # Extract timespan 1 DoDs
    DoD_stack_bool = array_to_bool(DoD_stack, 'semi-bool')
    DoD = DoD_stack_bool[run_index-1,:,:]
    
    # IMPORT envBAA INTENSITY MAP ---------------------------------------------
    envBAA_intensity = np.load('/home/erri/Documents/PhD/Research/5_research_repos/PiQs_analysis/activity_stack/' + run_name + '_envBAA_act_mean_intensity_LR5.npy')
    ...
    if env_tscale_array==[2,3,4,9, 20]:
    ...
    # plt.hist(envBAA_intensity_rsh.flatten(), bins=100, density=True, alpha=0.5, color='yellow', edgecolor='black', label='reshaped')
    # plt.hist(envBAA_intensity_rsh_filt.flatten(), bins=100, density=True, alpha=0.5, color='blue', edgecolor='black', label='reshaped and\nfiltered')
    ...
    # APPEND envBAA ACTIVE PERIOD MAP AS FRACTION OF THE RUN DURATION
    ...
    DoD_plot_img.save(os.path.join(report_dir, 'surveys_images_overlapping_test', run_name + '_DoD_resized_image.tiff'))
    
    envBAA_plot_img = Image.fromarray(np.array(envBAA_plot*1).astype(np.uint16))
    envBAA_plot_img.save(os.path.join(report_dir, 'surveys_images_overlapping_test', run_name + '_envBAA_resized_image.tiff'))
    ...
    if 'full_BAA_stack' in run_mode:
        plt.savefig(os.path.join(report_dir, 'surveys_images_overlapping_test', run_name + '_DEM_DoD_envBAA_overlapping.tiff'), dpi=600)
    elif 'partial_envBAA_stack' in run_mode:
        plt.savefig(os.path.join(report_dir, 'surveys_images_overlapping_test', run_name + '_DEM_DoD_partial_envBAA_overlapping.tiff'), dpi=600)
...
# STACK------------------------------------------------------------------------
    DEM_DoD_envBAA_stack = np.stack([DEM_rsz_rsc, DoD_rsz_rsc, envBAA_act_period_rsh, envBAA_intensity_rsh_filt], axis=0)
...
np.save(os.path.join(report_dir, set_name + '_envBAA_intensity_maps_single_runs.npy'), envBAA_set_name_stack)

# Number of active 1/4 of the run duration. (4 means 4/4=1 always active)
envBAA_act_period_stack = np.stack(envBAA_act_period_list)
np.save(os.path.join(report_dir, set_name + '_envBAA_act_period_maps_single_runs.npy'), envBAA_act_period_stack)
...
for set_name in ['q05_1']:
    print(set_name + ' is running...')
    
    # IMPORT DATA -------------------------------------------------------------
    envBAA_intensity_maps = np.load(os.path.join(report_dir, set_name + '_envBAA_intensity_maps_single_runs.npy')) # Bedload mean intensity
    envBAA_active_period_maps = np.load(os.path.join(report_dir, set_name + '_envBAA_act_period_maps_single_runs.npy')) # Number of active 1/4 of the run duration. (4 means 4/4=1 always active)
    DoD_maps    = np.load(os.path.join(home_dir, 'output','DoDs', 'DoDs_stack',"DoD_stack_"+set_name+".npy"))
...
for set_name in set_names:
    ...
    # IMPORT STACK ------------------------------------------------------------
    # DoD maps stack
    DoD_maps_stack    = np.load(os.path.join(home_dir, 'output','DoDs', 'DoDs_stack',"DoD_stack_"+set_name+".npy"))
    # Bedlaod mean intensity maps stack
    envBAA_stack = np.load(os.path.join(home_dir, 'output', set_name + '_envBAA_1Txnr_stack.npy')) # envBAA maps envelope at 1 Exner time
    ...
    for t in range(0,compensation_stack.shape[0]):
        ...
        plt.savefig(os.path.join(home_dir, 'output', set_name + '_' + str(t) +
