...
# runs = ['q20r9']
# runs = ['q10rgm2']
thrs_act = 8 # Active threshold - For values equal or grater than thrs_act bedeload is considered as active
W_tot = 620 # Total channe width calculated from images
survey_length = 6.140 # [meters]
smoothing_iter = 2 # Smoothin function iterations
...
for run in runs:
    print(run)
    # Setup data folder
    path_diff = os.path.join(folder_home, 'Differences', run) # Set the directory path where to pick up images
    path_img = os.path.join(folder_home, 'Photos', run)
    path_report = os.path.join(folder_home, 'output_report', run)
    path_blurry_area = os.path.join(folder_home, 'blurry_area_images')
    path_shaded_imgs = os.path.join(folder_home, 'shaded_area_images')
    ...
    if not(os.path.exists(path_shaded_imgs)):
        os.mkdir(path_shaded_imgs)
    if not(os.path.exists(os.path.join(path_shaded_imgs, run))):
        os.mkdir(os.path.join(path_shaded_imgs, run))
...
#%%
    ...
    for i in range(0,len(photo_names[:-2])): # For each images in the folder...
        ...
        # indx = int(np.where(diff_names==name)[0]) # Define a index


        # mean_delta_s = np.append(mean_delta_s, np.mean(diff_arr))
        
        # Image.fromarray(np.array(diff_arr)).save(os.path.join(diff_path_out, run, run + '_' + str(name)[:-4] + '.tiff'))
            
        # index = int(np.where(diff_names==name)[0]) # Define an index to connect differences and images
