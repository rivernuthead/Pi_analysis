...
Inputs:
---------
    1. Difference of saturation maps 
    2. Borders masks : Mask of the flume's borders
    3. Mask: Upstream and dowstream image masks
    
Outputs:
---------
    1. Filtered bedload activity maps at the highest resolution
    2: Filtered bedload activity maps at lower resolution (5)
    Optional:
    3. Filtering steps maps as tif files
...
# SCRIPT MODE -----------------------------------------------------------------
plt_show = 0
compute_DoS_envelops = 0 # Compute the Difference of saturation envelope as the mean of DoS for each run

# SCRIPT PARAMETERS -----------------------------------------------------------
dt = 1 # Time between shoots in minutes (for plt_show = 1)
threshold_sensitivity_analysis = 0 # the value is added to the thrs_act, if 0 no sensitivity analysis
save_noise_filtering = 0 # if 1 save the noise filtered map into a .tiff file (step 1)
save_diff_averaging = 0 # if 1 save the diff average map into a .tiff file (step 2)
save_thrs_maps = 0 # if 1 save filtered maps using the upstream and downstream thresholds into a .tiff file (step 3)
save_rso_maps = 0 # if 1 save the maps after the remove small object step into a .tiff file (step 4)
...
for run_index, run in enumerate(runs):
    ...
    diff_path_out = os.path.join(path_output_data, '1_PiQs_BAW_maps',set_names[run_index])
...
# 
    for diff_index,name in enumerate(diff_names): # For each images in the folder...
        ...
        if threshold_sensitivity_analysis>0:
            thrs_actDS = thrs_actDS + threshold_sensitivity_analysis
            thrs_actUS = thrs_actUS + threshold_sensitivity_analysis
