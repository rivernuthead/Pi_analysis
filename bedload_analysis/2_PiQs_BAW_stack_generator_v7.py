...
for run in runs:
    ...
    for name in BAA_maps_names: # For each images in the folder...
        ...
        BAA_map_path = os.path.join(diff_path_out, run, name) # Define the BAA map path
    ...
    np.save(os.path.join(folder_home, 'activity_stack', run + '_BAA_stack_LR' +
            str(downsampling_dim) + '.npy'), np.around(BAA_map_stack_bool, decimals=0))
    ...
    np.save(os.path.join(folder_home, 'activity_stack', run + '_BAA_stack_envelope_LR' +
    ...
    np.save(os.path.join(folder_home, 'activity_stack', run + '_envBAA_act_period_LR' +
    ...
    np.save(os.path.join(folder_home, 'activity_stack', run + '_envBAA_act_mean_intensity_LR' +
...
for subfolder_filter, set_name in zip(['07', '10', '15', '20'], ['q07_1', 'q10_2', 'q15_2', 'q20_2']):
    
    # Collect and stack the files
    npy_stack = collect_and_stack_files(
        base_dir, subfolder_filter, file_filter, exclude_filter)
    
    np.save(os.path.join(os.getcwd(), 'activity_stack', set_name +
